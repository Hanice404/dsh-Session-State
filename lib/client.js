...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---------------------------------------------------------------------
    var BAR_CSS = [
      // Left edge aligned with the input box (composer card) via JS-measured
      // margin-left; width is also set to the card's width from JS. The
      // `--dsh-composer-card-max-width` variable cannot be trusted here because
      // other plugins (e.g. dsh-width) override it.
      '.dsm-bar{box-sizing:border-box;width:100%;margin-left:0;margin-right:auto;padding:0 10px;display:flex;flex-direction:column;align-items:stretch;flex:none}',
    ...
    function SessionStatusBar(props) {
      ...
      // other plugins (dsh-width) and no pure-CSS formula survives that.
      ...
      return React.createElement('div', { ref: barRef, className: 'dsm-bar', 'data-dsm-statusbar': '' }, [
        ...
        }, [runDot, React.createElement('span', { key: 'chips', className: 'dsm-chips' }, chips), React.createElement('span', {
          key: 'chevron',
