...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var FOLD_CSS_ID = 'dsh-session-manage/fold';
    ...
    // ---------------------------------------------------------------------
    var BAR_CSS = [
      // Both edges aligned with the input box (composer card): the JS-measured
      // margin-left + width pin the bar to the card's exact box, and the bar
      // itself has no horizontal padding so the toggle and the expanded panel
      // both span the card's full width, flush with its left and right edges.
      // `--dsh-composer-card-max-width` cannot be trusted here because other
      // plugins (e.g. dsh-width) override it.
      ...
      '.dsm-toggle{display:flex;align-items:center;justify-content:space-between;gap:6px;width:100%;min-height:24px;padding:2px 10px;border:1px solid transparent;border-radius:10px;background:transparent;color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px;cursor:pointer;text-align:left}',
    ...
    function SessionStatusBar(props) {
      ...
      React.useLayoutEffect(function () {
        ...
        if (card === null || host === null) return undefined;

        var align = function () {
          var cardRect = card.getBoundingClientRect();
          var hostRect = host.getBoundingClientRect();
          var left = Math.max(0, Math.round(cardRect.left - hostRect.left));
      ...
      return React.createElement('div', { ref: barRef, className: 'dsm-bar', 'data-dsm-statusbar': '' }, [
        React.createElement('button', {
          key: 'toggle',
          type: 'button',
